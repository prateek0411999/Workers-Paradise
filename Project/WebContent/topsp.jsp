...
<br><br><br>
<h2 class="text-center">TOP 5 SERVICE PROVIEDERS</h2>
<hr>
<br><br>
<%
...
try{
	...
	stmt=con.prepareStatement("select   spuid,cntbid   from (select   spuid,count(bid) as cntbid  from tblbooking group by (spuid)  ) order by cntbid desc");
    rset=stmt.executeQuery();
    System.out.println("???????????????????????????????????????????????");
    for(int i=0;i<5;i++){
    	rset.next();
    	strspid=rset.getString("spuid");
        totalorder=rset.getInt("cntbid");
        con1=obj1.getDBconnection();
        System.out.println(strspid);
        stmt1=con1.prepareStatement("select * from tblserviceprovider where spuid=?");
        stmt1.setString(1, strspid);
        System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
